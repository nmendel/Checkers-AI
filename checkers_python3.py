# This file is from https://pypi.python.org/pypi/checkers/0.2
# It is a checkers UI and was edited slightly by Nick Mendel
# to make it possible to add in his AI (in play.py)
...
#++++++++++++++++++++++++++++++++++++++++more detailed functions+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    def setup_move(self):
        ...
        if self.DEBUG_PRINT_FUNCTIONS:
            pprint("setup_move")
        if self.DEBUG:
            pprint("lengh of history:", len(self.history))
            pprint("count:", self.count)

        #kingmaker part
        not_crowning=1
        for piece in self.pieces[self.moving]:
            if self.DEBUG_BIG_THINGS:
                pprint(self.c.coords(piece))
        ...
        if self.DEBUG_BIG_THINGS:
            pprint(self.jumps[0], self.jump_made)
        same_piece=0
    ...
    def check_move(self):
        ...
        if self.DEBUG_PRINT_FUNCTIONS:
            pass; pprint("check_move")

        if len(self.square) != 1 or self.piece == None:
            if self.DEBUG:
                pprint("missing piece or square!")
            return 5
        ...
        if self.jumps[0]: #jump checker
            ...
            if self.jumps[0].count((self.piece, vtr)) != 1:
                ...
                if self.DEBUG:
                    pprint("jump_made: ", self.jump_made)
                return 0
        ...
        if self.c.itemcget(self.piece, "outline") != "gold2":
            if self.moving== "black":
                if vtr[1] > 0:
                    if self.DEBUG:
                        pprint("wrong way, black!")
                    return 3
            else:
                if vtr[1] < 0:
                    if self.DEBUG:
                        pprint("wrong way, red!")
                    return 3
        ...
        if self.c.type(self.c.find_overlapping(sqr_cords[0]+(self.SQUARESIZE/2), \
           ...
           if self.DEBUG:
               pprint("not empty: ",self.c.find_overlapping(sqr_cords[0]+(self.SQUARESIZE/2), \
                                               sqr_cords[1]+(self.SQUARESIZE/2), \
                                               sqr_cords[2]-(self.SQUARESIZE/2), \
...
#++++++++++++++++++++++++++++++++++++++++++++++helper functions++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def go_to_move(self, event=None, move_number=None,):
        ...
        if self.DEBUG:
            pprint("move_number:", move_number)
        if move_number < 0:
            return 1
